...
class CommandResponseHandler(ResponseHandler):
    ...
    def _build_response(self, response, response_type_opt, update):
        return _CommandExecutor.execute(response, response_type_opt, update)
    ...
    async def _handle_response(self, bot, update, response, response_type_opt) -> None:
        if self._keyboard_markup:
            await bot.send_message(
                ...
                response,
                parse_mode=AutoBotConstants.PARSE_DICT[
                    self._entry[AutoBotConfig.RESPONSE_PARSE_MODE]
                ],
    ...
    def check_options(cls, entry_id, response_type_opt):
        if isinstance(response_type_opt, dict):
            if ResponseParseMode.SEND_EXIT_CODE in response_type_opt:
                if not isinstance(
                    response_type_opt[ResponseParseMode.SEND_EXIT_CODE], bool
                ):
                    raise TypeError(
                        """"send_exit_code" in "{}" entry,
                    must be a boolean""".format(
                            entry_id
                        )
                    )
            if ResponseParseMode.SEND_STDOUT in response_type_opt:
                if not isinstance(
                    response_type_opt[ResponseParseMode.SEND_STDOUT], bool
                ):
                    raise TypeError(
                        """"send_stdout" in "{}" entry,
                    must be a boolean""".format(
                            entry_id
                        )
                    )
            if ResponseParseMode.SEND_STDERR in response_type_opt:
                if not isinstance(
                    response_type_opt[ResponseParseMode.SEND_STDERR], bool
                ):
                    raise TypeError(
                        """"send_stderr" in "{}" entry,
                    must be a boolean""".format(
                            entry_id
                        )
                    )
...
class _CommandExecutor(object):
    ...
    def execute(cls, command, options, update):
        ...
        try:
            completed_proc = subprocess.run(
                command if onshell else command.split(),
                input=None,
                timeout=timeout,
                check=False,
                stdout=stdout,
            ...
            cls._logger.debug(
                'Execution of "%s" termined with code: %i',
                command,
        ...
        ):  # TODO aggiungere le altre eccezioni che subprocess.run puo' lanciare (permessi insufficienti)
    ...
    def _build_response(cls, completed_proc, options):
        ...
        if completed_proc.stdout:
            message_str += completed_proc.stdout.decode(
                "utf-8"
            )  # TODO utf-8 configurabile

        if completed_proc.stderr:
            message_str += completed_proc.stderr.decode(
                "utf-8"
            )  # TODO utf-8 configurabile
