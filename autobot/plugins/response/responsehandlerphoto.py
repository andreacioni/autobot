...
class PhotoResponseHandler(ResponseHandler):
    ...
    async def _handle_response(self, bot, update, response, response_type_opt) -> None:
        if isinstance(response, URLError):
            await bot.send_message(update.message.chat_id, str(response))
        else:
    ...
    def _get_request(self, response, response_type_opt):
        if (
            ResponseOptional.AUTH in response_type_opt
            ...
            if auth[AuthOptional.TYPE].upper() == "BASIC":
                return self._get_basic_auth_request(
                    response, auth[AuthOptional.USERNAME], auth[AuthOptional.PASSWORD]
                )
    ...
    def check_options(cls, entry_id, response_type_opt):
        if (
            ...
            raise TypeError(
                'Caption length in "PHOTO" must be less than 200 characters'
            )
