...
class AutoBotConfig(object):
    ...
    def load(self):
        ...
        for value in self._config.values():
            del dict(value)[
                AutoBotConfig.ID
            ]  # Deleting AutoBotConfig.ID field from a new copy of 'value' dictionary
    ...
    def _check_base_attribute(self, entries):
        ...
        for entry in entries:
            ...
            if AutoBotConfig.ID not in entry:
                ...
                raise AttributeError(
                    "No 'handler_type' attribute found inside entry {}".format(
                        entry[AutoBotConfig.ID]
                    )
                )
            elif AutoBotConfig.ON not in entry:
                raise AttributeError(
                    "No 'on' attribute found inside entry {}".format(
                        entry[AutoBotConfig.ID]
                    )
                )
            elif (AutoBotConfig.RESPONSE not in entry) or (
                entry[AutoBotConfig.RESPONSE].strip() == ""
            ):
                raise AttributeError(
                    "No 'response' attribute found inside entry {}, or it is empty".format(
                        entry[AutoBotConfig.ID]
                    )
                )
            ...
            if entry[AutoBotConfig.HANDLER_TYPE] not in AutoBotConstants.HANDLER_TYPES:
                raise TypeError(
                    '"{}" as handler_type is not valid. Allowed {}'.format(
                        entry[AutoBotConfig.HANDLER_TYPE],
                        AutoBotConstants.HANDLER_TYPES,
                    )
                )

            if (AutoBotConfig.RESPONSE_PARSE_MODE in entry) and (
                entry[AutoBotConfig.RESPONSE_PARSE_MODE]
                not in AutoBotConstants.PARSE_DICT
            ):
                raise TypeError(
                    '"{}" as response_parse_mode is not valid. Allowed {}'.format(
                        entry[AutoBotConfig.RESPONSE_PARSE_MODE],
                        AutoBotConstants.PARSE_DICT,
                    )
                )
            # If GROUP is defined it must be a list. At least it can contains only one item
            if (AutoBotConfig.GROUP in entry) and not (
                isinstance(entry[AutoBotConfig.GROUP], list)
            ):
            ...
            if (
                (AutoBotConfig.EXIT_POINT in entry)
                and (AutoBotConfig.TO_GROUP in entry)
                and entry[AutoBotConfig.EXIT_POINT]
            ):
                raise AttributeError(
                    '"{}" is ignored when "{}" is set to "true", in entry "{}"'.format(
                        AutoBotConfig.TO_GROUP,
                        AutoBotConfig.EXIT_POINT,
                        entry[AutoBotConfig.ID],
                    )
                )
        ...
        on_backslash = [
            ...
            for entry in entries
            if entry[AutoBotConfig.ON].startswith("/")
        ]
        if on_backslash:
            raise AttributeError(
                "Found at least one 'on' contains '/'."
        ...
        text_lenght = [
            entry[AutoBotConfig.ID]
            for entry in entries
            if (len(entry[AutoBotConfig.ON]) > AutoBotConstants.MAX_MESSAGE_LENGTH)
            or (
                ("invalid_option_message" in entry)
                and (
                    len(entry["invalid_option_message"])
                    > AutoBotConstants.MAX_MESSAGE_LENGTH
                )
            )
        ]
        if text_lenght:
            raise AttributeError(
                "Found text message lenght > "
                + str(AutoBotConstants.MAX_MESSAGE_LENGTH)
                + ' inside: "'
                + str(text_lenght)
                + '"'
            )
    ...
    def _check_command_extra_attributes(self, entry):
        if (
            AutoBotConfig.RESPONSE_TYPE in entry
            and AutoBotConfig.RESPONSE_TYPE_OPTIONS in entry
        ):
            response_type = entry[AutoBotConfig.RESPONSE_TYPE]
            if PluginsLoader.has_response_handler(response_type):
                response_handler_class = PluginsLoader.get_response_handler_class(
                    response_type
                )
                response_handler_class.check_options(
                    entry[AutoBotConfig.ID], entry[AutoBotConfig.RESPONSE_TYPE_OPTIONS]
                )
            else:
                raise KeyError(
                    "Not a valid response type ({}) defined in entry: {}".format(
                        response_type, entry[AutoBotConfig.ID]
                    )
                )
        ...
        for entry in entries:
            ...
            if entry[AutoBotConfig.ENTRY_POINT] and not entry[AutoBotConfig.TO_GROUP]:
                if len(entry[AutoBotConfig.GROUP]) == 1:
                    ...
                    raise RuntimeError(
                        'Found "{}" starting point belong to multiple groups and no "TO_GROUP" defined'.format(
                            entry[AutoBotConfig.ID]
                        )
                    )
        ...
        if (
            ...
            if PluginsLoader.has_response_handler(response_type):
                ...
                entry[AutoBotConfig.RESPONSE_TYPE_OPTIONS] = (
                    response_handler_class.set_default_options(
                        entry[AutoBotConfig.ID],
                        entry[AutoBotConfig.RESPONSE_TYPE_OPTIONS],
                    )
                )
    ...
    def _load_entries(self):
        ...
        ret_array = []
        files = glob.glob(
            self._configuration_dir + os.sep + "*.json"
        )  # TODO case insensitive extension needed
        ...
        for file_path in files:
            ...
            with open(file_path, encoding="utf-8") as json_fp:
                ...
                if not json_obj or not isinstance(json_obj, list):
                    raise TypeError(
                        "Not valid JSON array found in {} file".format(file_path)
                    )
                else:
                    self._logger.info(
                        "Found %i item/s in %s file", len(json_obj), file_path
                    )
